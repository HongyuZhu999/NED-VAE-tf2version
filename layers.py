...
def dropout_sparse(x, keep_prob, num_nonzero_elems):
    ...
    random_tensor += tf.random_uniform_initializer(noise_shape)  # Changed by hyz
    dropout_mask = tf.cast(tf.floor(random_tensor), dtype=tf.bool)
    # pre_out = tf.sparse_retain(x, dropout_mask)
    pre_out = tf.sparse.retain(x, dropout_mask)  # Changed by hyz
...
class Graphite(Layer):
    ...
    def _call(self, inputs):
        ...
        x = tf.matmul(recon_1, tf.matmul(tf.transpose(recon_1, [0, 2, 1]), x))\
            + tf.matmul(recon_2, tf.matmul(tf.transpose(recon_2, [0, 2, 1]), x))
        outputs = self.act(x)
        return outputs
...
class batch_norm(object):
    # h1 = lrelu(tf.contrib.layers.batch_norm(conv2d(h0, self.df_dim*2, name='d_h1_conv'),
    ...
    def __call__(self, x, train=True):
        return tf.compat.v1.layers.batch_normalization(x, momentum=self.momentum,
                                                       epsilon=self.epsilon, scale=True, name=self.name)
        # Changed by hyz
