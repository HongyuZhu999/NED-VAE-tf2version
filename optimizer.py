...
class OptimizerVAE(object):
    def __init__(self, preds_edge, preds_node, labels_edge, labels_node, model, num_nodes, pos_weight, norm, beta):
        ...
        self.cost = norm*tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(logits=preds_edge, labels=labels_edge,
                                                                                 pos_weight=pos_weight))
        self.cost += norm * tf.reduce_mean(tf.math.squared_difference(labels_node, tf.reshape(preds_node, [-1, 1])))
        mse_loss = norm*tf.reduce_mean(tf.nn.weighted_cross_entropy_with_logits(logits=preds_edge, labels=labels_edge,
                                                                                pos_weight=pos_weight)) +\
                                tf.reduce_mean(tf.math.squared_difference(labels_node, tf.reshape(preds_node, [-1, 1])))
        # In tf2.0+ targets = xxx -> labels = xxx
